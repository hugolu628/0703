...
class SalaryInput(BaseModel):
    years_experience: float = Field(..., description="工作經驗年數")
    education_level: str = Field(..., description="教育程度")
    city: str = Field(..., description="城市")


class SalaryOutput(BaseModel):
    predicted_salary: float = Field(..., description="預測每月薪資")
    estimated_annual_salary: float = Field(..., description="估計年薪 (含獎金等) 結果")
